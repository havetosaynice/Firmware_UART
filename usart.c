...
uint16_t Uart1_RX_Len = 0;  // 接收数据长度    （没用）
uint8_t  Uart1_RX_Flag = 0; // 接收完成标志位			（没用）
//发送
uint8_t  Uart1_TX_Flag = 0; // 发送完成标志位
uint8_t  Uart1_TX_Busy = 0; // 发送忙标志位
...
void HAL_UART_TxCpltCallback(UART_HandleTypeDef *huart)
{
    if(huart == &huart1)
    {
        uart1_tx_flag = 1;  // 置位发送完成标志位，main函数可检测
        uart1_tx_busy = 0;  // 发送完成，清除发送忙标志位
    }
}
//串口空闲回调函数，从接收转到空闲时候
...
{
	CRC_USER Crc;
	const uint8_t  RequesData[4] = {0x01, PackageIndex,}; // DMA发送缓冲区
	...
	Uart1_TX_Flag = 0;               // 发送完成标志位
	Uart1_TX_Busy = 1;               // 发送忙标志位
}
